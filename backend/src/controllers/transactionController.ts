...
async function autoGenerateReceipt(transactionId: string): Promise<void> {
  try {
    ...
    const receiptNumber = `PL-${new Date().getFullYear()}-${String(count + 1).padStart(6, '0')}`;
    ...
    await prisma.receipt.create({
      data: {
        ...
        paymentTime: transaction.paidAt || new Date(),
    ...
    console.log(`✅ Receipt ${receiptNumber} auto-generated for transaction ${transactionId}`);
  } catch (err) {
    console.error('Auto-generate receipt error (non-fatal):', err);
  }
}
