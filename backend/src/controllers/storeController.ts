...
export const updateStoreStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (status === 'FROZEN') {
      return res.status(403).json({ success: false, error: 'Store cannot be frozen by seller' });
    }

    // Basic readiness check: store must have a name and slug
    if (status === 'ACTIVE') {
      if (!store.name || !store.slug) {
        return res.status(400).json({
          success: false,
          error: 'Store must have a name and URL slug before it can be activated',
        });
      }
    }
